...
const route = req => {
  ...
  if(page.endsWith('/')) { page = page + 'index.js' }
  if(!page.endsWith('.js') || page.endsWith('.json')) { page = page + '.js' }
...
const renderGet = async (req, res, settings) => {
  ...
  try {
    ...
    const html = await render(req, res, path, query, {page, buildInfo})
    res.end(html)
  } catch (err) {
    console.log('Error', err)
    res.statusCode = 500
    try {
      const html = await render(req, res, path, query, {err, page: '/_error.js', buildInfo, settings})
      res.end(html)
    } catch (err2) {
